...
@elseif ($product->type == 'configurable')
    ...
    @push('scripts')
        ...
        <script>
            var variants1 = @json(app('Webkul\PreOrder\Helpers\Product')->getPreOrderVariants($product));
            
            eventBus.$on('configurable-variant-selected-event', function(variantId) {
                var a = variants1[variantId];

                if (typeof variants1[variantId] != "undefined") {
                    $('.preorder-info').show()
                    ...
                    $('.preorder-info .availability-text').html(variants1[variantId]['availability_text'])
                } else {
                    $('.preorder-info').hide()
