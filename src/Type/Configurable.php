...
class Configurable extends AbstractType
{
    ...
    public function isItemHaveQuantity($cartItem)
    {
        ...
        if ( core()->getConfigData('preorder.settings.general.enable_preorder') ) {
            $childProduct = $cartItem->child->product;
            $productFlat = $childProduct->product_flats->first();

            if ( $childProduct->type == 'simple' 
                && isset($productFlat->allow_preorder) 
                && $productFlat->allow_preorder 
                && $childProduct->totalQuantity() < 1) {
                return true;
            } else {
    ...
    public function prepareForCart($data)
    {
        ...
        // Bagisto PreOrder Check
        if ( core()->getConfigData('preorder.settings.general.enable_preorder') ) {
            $productFlat = $this->product->product_flats->first();
    ...
    public function haveSufficientQuantity(int $qty): bool
    {
        ...
        foreach ($this->product->variants as $variant) {
            // Bagisto PreOrder Check
            if ( core()->getConfigData('preorder.settings.general.enable_preorder') ) {
                $mainProduct = $this->product->product_flats->first();

                if ( isset($mainProduct->allow_preorder) && $mainProduct->allow_preorder ) {
